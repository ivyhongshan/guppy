...
const anagineRouter = async (req, res, next) => {
  const {
    type, filter, sort, fields, accessibility,
  } = req.body;

  log.debug('[anagine] ', JSON.stringify(req.body, null, 4));
  ...
  try {
    ...
    switch (tierAccessLevel) {
      ...
      case 'regular': {
        log.debug('[anagine] regular commons');
        if (accessibility === 'accessible') {
          ...
          if (outOfScopeResourceList.length > 0) {
            log.info('[anagine] requesting out-of-scope resources, return 401');
            log.info(`[anagine] the following resources are out-of-scope: [${outOfScopeResourceList.join(', ')}]`);
